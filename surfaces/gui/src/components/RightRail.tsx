...
function ProgressSummary({ running, toolNames, todo }: { running: boolean; toolNames: string[]; todo: TodoItem[] }) {
  ...
  if (running) {
    return (
      <div className="rail-muted">
        {toolNames.length ? t("Working on this task with {{count}} tool calls so far.", { count: toolNames.length }) : t("Working on this task.")}
      </div>
    );
  }
  return (
    <div className="rail-muted">
      {t("For longer multi-step tasks, progress will appear here while OpenWorker plans, uses tools, waits for approval, and produces artifacts.")}
    </div>
  );
}
      ...
      <div className="artifact-preview">
        {!content ? (
          ...
          <div className="artifact-open-prompt">
            ...
            <p>{/\.pptx?$/i.test(artifact.name) ? t("This PowerPoint file can’t be previewed here.") : t("This Word file can’t be previewed here.")}</p>
