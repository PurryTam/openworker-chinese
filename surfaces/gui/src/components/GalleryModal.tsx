    ...
    <div data-testid="gallery-cards">
      ...
      {unavailable && cloud?.signed_in && (
        <div className="text-[12.5px] text-muted">
          {t("The gallery is unreachable right now — try again in a moment.")}
        </div>
      )}
            ...
            <div className={CARD + " p-4"} data-testid="gallery-capabilities">
              ...
              <div className="space-y-2 text-[12.5px]">
                ...
                <div>
                  ...
                  {caps.mcp.length > 0 ? " · " + t("MCP: {{mcp}}", { mcp: caps.mcp.join(", ") }) : ""}
                </div>
        ...
        <div className="overflow-y-auto hairline-scroll p-5">
          {loading ? (
            ...
            <div className={CARD + " p-5 flex items-center gap-4"} data-testid="gallery-signin">
              <div className="min-w-0 flex-1">
                <div className="font-semibold text-[14px] mb-1">{t("Sign in to browse the Gallery")}</div>
                <div className="text-[12.5px] text-muted leading-relaxed">
                  {t("The Gallery is a curated set of coworkers from OpenWorker Cloud and needs a (free) cloud sign-in. Installing personas from a folder or Git URL — on the Personas page — always works without an account.")}
                </div>
              </div>
              <button className={BTN_ACCENT} onClick={signIn} disabled={signingIn}>
                {signingIn ? t("Check your browser…") : t("Sign in")}
              </button>
            </div>
          ) : detailSlug ? (
            detailView
