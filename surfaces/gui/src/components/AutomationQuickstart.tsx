      ...
      <div className="grid grid-cols-3 auto-rows-fr gap-3">
        {TEMPLATES.map((tmpl) => (
          <button
            ...
            onClick={() => pick(tmpl)}
          >
            <span className="text-[13.5px] font-semibold">{t(tmpl.title)}</span>
            <span className="text-[12px] text-muted leading-relaxed flex-1">{t(tmpl.blurb)}</span>
            <span className="flex items-center gap-1.5 mt-1">
              {tmpl.conns.map((c) => {
                ...
                return (
                  <span
                    key={c.name}
                    title={`${cs?.title || c.name} — ${on ? t("connected") : t("not connected yet")}`}
              ...
              <span className="text-[11px] text-faint ml-0.5">
                {tmpl.conns.length === 0 ? t("No connections needed · ") + t(tmpl.cadence) : t(tmpl.cadence)}
              </span>
            </span>
          </button>
        ))}
        ...
        >
          ...
          {pendingConn && !cloud?.signed_in && (
            ...
            >
              ...
              {t("Connections are brokered by OpenWorker Cloud — your tokens stay on this Mac.")}
            ...
            <div className={picked.conns.length ? "bg-paper rounded-xl px-4 py-3.5 mt-3" : ""} data-testid="ob-recipe">
              ...
              {picked.consent ? (
                <label className="flex items-start gap-2.5 mt-3.5 text-[12.5px] text-muted select-none">
                  ...
                  />
                  <span>
                    {t("Allow this automation to post its digest to ")}<b
                      className="text-ink"
                      title={channel || undefined}
                    >
