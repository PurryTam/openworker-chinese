                    ...
                    <div className="flex items-center gap-2 py-1" key={c.connector}>
                      ...
                      <div className="min-w-0 flex-1">
                        ...
                        {byName[c.connector]?.channels && (
                          ...
                          >
                            {t("Channels · ")}{channelsOf(c.connector).length}
...
function ConnectInline({
  ...
  return (
    <div>
      ...
      >
        <Icon name="arrowLeft" size={13} /> {t("Connect")} {c.title}
      </button>
      ...
      <p className="text-[10.5px] text-faint mt-2 leading-snug">
        {t("Connecting makes {{title}} available to all your coworkers — the toggle in this list controls just this session.", { title: c.title })}
      </p>
    </div>
  );
}
...
function ChannelsInline({
  ...
  return (
    <div>
      ...
      <div className={`${SEC_H} mb-1.5`}>{t("Subscribed channels · ")}{channels.length}</div>
      {channels.length === 0 ? (
        <div className="text-[12px] text-faint py-0.5">
          {t("Not listening to any {{label}} channel yet.", { label })}
        </div>
      ) : (
        <div className="space-y-1">
          {channels.map((s) => (
