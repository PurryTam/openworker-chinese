...
function approvalChip(t: (k: string, o?: Record<string, unknown>) => string, resolved: ApprovalDecision | undefined) {
  if (resolved === "deny")
  ...
  return (
    <span
      ...
      title={resolved ? t("approved · {{r}}", { r: resolved.replace(/_/g, " ") }) : t("approved")}
    >
      ✓ {t("approved")}
    </span>
  );
...
function TurnGroup({
  ...
  const stepsLabel = nSteps === 1 ? t("1 step") : t("{{count}} steps", { count: nSteps });
          ...
          {rows.map((row, i) =>
            row.type === "narr" ? (
              ...
              <div className="flex items-baseline gap-2 px-2 py-0.5" key={i} data-testid="turn-ask">
                ...
                {approvalChip(t, row.approval.resolved)}
              </div>
            ) : (
