    ...
    <Shell>
      <div className="flex items-start gap-3">
        <div className="flex-1 min-w-0">
          <PanelHead title={t("Automations")} sub={t("Recurring tasks OpenWorker runs on a schedule.")} />
        </div>
        <button
      ...
      {empty ? (
        !showForm && (
          <div className={CARD + " p-4 text-[12.5px] text-muted"}>
            {t("No scheduled tasks yet — use a template above, click ")}<strong>{t("+ New automation")}</strong>{t(", or just ask OpenWorker in a session.")}
          </div>
        )
      ) : (
          ...
          {tasks.map((task) => (
            ...
            >
              ...
              <div className="flex items-center gap-1.5 text-[12px] text-muted">
                ...
                {task.enabled ? task.schedule : t("Paused")} · next {fmt(task.next_run)} · {task.run_count} run{task.run_count === 1 ? "" : "s"}
      ...
      <div className="sched-detail">
        ...
        {editing ? (
          ...
          <div className="conn-meta">
            ...
            {task.enabled ? t("Active · next ") + fmt(task.next_run) : t("Paused")} · {task.schedule}
          </div>
        )}
          ...
          >
            <div className="sched-run-row">
              <span>
                ...
                {r.artifacts.length > 0 && <span className="dim"> · {r.artifacts.length} {t("file(s)")}</span>}
              </span>
